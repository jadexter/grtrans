...
!          dthdx2=pi*(1.+(1.-hval)*cos(2.*pi*x2))
        ...
        subroutine harmpi_vals(x0,a,rho,p,b,u,bmag)
...
!        defcoord=header(19)
        ...
        subroutine read_harmpi_data_file(data_file,tcur,rho,p,u,b,mdot)
        character(len=100), intent(in) :: data_file
        character(len=100) :: data_file_app
!        character, dimension(:), allocatable :: header_bytes
...
!          rhopos=6; ppos=7; vpos=14; bpos=22; gdetpos=30
          rhopos=9; ppos=10; vpos=19; bpos=27
...
!          write(6,*) 'after read loop'
             ...
             read(8) data
             close(8)
             write(6,*) 'harmpi read done with header: ',nheader_bytes
             rho=data(rhopos+1,:)
!                write(6,*) 'sizes rho: ',size(rho),size(data(rhopos+1,:))
             p=data(ppos+1,:)
...
!             call read_harmpi_data(data_file_app,rho,p,u%data(1),u%data(2),u%data(3),u%data(4),b%data(1), &
!                  b%data(2),b%data(3),b%data(4))
!             call read_harmpi_grid_file()
...
! test code...
        ...
        subroutine read_harmpi_grid_file()
...
!        n=nx1*nx2*nx3
        ...
        subroutine read_harmpi_data(dfile,rho,p,u0,vr,vth,vph,b0,br,bth,bph)
        integer(kind=4) :: nx, status
...
!           write(6,*) 'ERROR: INCORRECT DATA SIZE IN READ_HARMPI_DATA: ',nx,nx1,nx2,nx3
!           return
!        endif
        nx=dlen*nx1*nx2*nx3
        write(6,*) 'pre data read size: ',nx,dlen,nx1,nx2,nx3
        allocate(data(nx))
        write(6,*) 'data allocated'
        read(8) data
        close(8)
        write(6,*) 'file closed'
...
!        u0=1./sqrt(-ui2)
        end subroutine read_harmpi_data
...
!        write(6,*) 'init harm'
        call read_harmpi_data_header(nhead)
...
!           write(6,*) 'data_file: ',indf-(k-1),append,data_file
           call read_harmpi_data_file(data_file,tcur,rho,p,u,b)
           t(k)=tcur
