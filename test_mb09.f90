  ...
  use fluid_model_mb09, only: initialize_mb09_model, mb09_vals, del_mb09_data
  ...
  nr=128; nz=128
  a=0.92; rmin=0.9*calc_rms(a); rmax=40.
...
!  write(6,*) 'after: ',size(r),size(z),size(r2d),size(z2d)
  ...
  call initialize_mb09_model(dble(a),1)
...
! Set up x four-vector:
  ...
  call mb09_vals(x0,a,rho,p,b,u,bmag)
...
! Output so I can plot in IDL
  ...
  open(unit=12,file='test_mb09.out')
  write(12,*) nr*nz, a
  write(12,*) r, z, phi
  write(12,*) rho, p
  write(12,*) b*b
...
!  write(6,*) 'norm: ', maxloc(norm)
  ...
  write(6,*) 'unit test mb09: ',maxval(norm),maxval(abs(ub)),minval(rho)
  open(unit=12,file='unit_test_mb09.out')
